...
class CentroidTracker():
    ...
    def update(self, rects):
        ...
        # 否则需要和旧人脸进行匹配
            ...
            # rows里存储的是旧人脸的ID排列顺序，按照“最小距离”从小到大排序
            rows = D.min(axis=1).argsort()

            # argmin(axis=1)获得每一行中的最小值的索引值，代表了距离每一个旧人脸最近的新人脸的ID
            # cols里存储的是新人脸的ID按照rows排列
            cols = D.argmin(axis=1)[rows]

            # 为了防止重复更新、注册、注销等操作，我们设置两个集合存储已经更新过的row和col
            usedRows = set()
            usedCols = set()

            # 对于所有(row,col)的组合执行以下操作
            for (row, col) in zip(rows, cols):
                # 若row和col二者之一被检验过，则跳过
